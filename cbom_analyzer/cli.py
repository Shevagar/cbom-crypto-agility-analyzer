...
def main():
    ...
    inventory={"schema":"cbom-analyzer/v0.5","summary":{
        "findings":len(data),"certificates":len(certificates),"key_material":len(keys),"hardware_crypto":len(hardware),
        "relationships":len(relationships),"migration_constraints":len(constraints),
      ...
      "hardware_crypto":[x.to_dict() for x in hardware],"relationships":[x.to_dict() for x in relationships],"migration_constraints":constraints}
