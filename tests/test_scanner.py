...
class ScannerTests(unittest.TestCase):
    ...
    def test_detects_key_size_and_pqc_review(self):
        ...
        rsa=next(x for x in findings if x.name=="RSA"); aes=next(x for x in findings if x.name=="AES")
        self.assertEqual(rsa.key_size,3072); self.assertTrue(rsa.quantum_vulnerable)
        self.assertEqual(aes.key_size,256); self.assertFalse(aes.quantum_vulnerable)

    def test_evp_cipher_and_rsa_patterns(self):
        findings=self.scan_text("EVP_aes_256_gcm();\nint type=EVP_PKEY_RSA;")
        aes=next(x for x in findings if x.name=="AES"); rsa=next(x for x in findings if x.name=="RSA")
        self.assertEqual(aes.key_size,256); self.assertIsNone(rsa.key_size)
    ...
    def test_detects_legacy_crypto(self):
        ...
        self.assertEqual(statuses["SHA-1"],"legacy"); self.assertEqual(statuses["MD5"],"deprecated")
    ...
    def test_infers_signature_purpose_for_nearby_rsa(self):
        ...
        self.assertEqual(rsa.purpose,"digital-signature"); self.assertEqual(rsa.migration_family,"ML-DSA / SLH-DSA")
